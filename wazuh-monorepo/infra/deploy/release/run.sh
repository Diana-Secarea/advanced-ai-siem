#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
#  Selene — launcher (Linux)
#
#  Starts the Selene backend + dashboard on http://127.0.0.1:5000.
...
if [ ! -x "$VENV/bin/python" ]; then
    echo "Selene isn't set up yet — run ./install.sh first." >&2
    exit 1
fi
...
printf '\033[1;36mSelene → http://%s:%s   (Ctrl+C to stop)\033[0m\n' "$BIND_HOST" "$BIND_PORT"
cd "$BACKEND"
