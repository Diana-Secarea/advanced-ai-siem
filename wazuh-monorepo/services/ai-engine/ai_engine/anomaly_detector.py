...
try:
    from attack_labels import is_routine_safe
...
class AnomalyDetector:
    ...
    def train_new_model(self):
        ...
        if len(training_data) > 0:
            ...
            keep = [i for i in range(X.shape[1]) if i not in self.IF_DROPPED_FEATURES]
            X = X[:, keep]
    ...
    # Features the Isolation Forest is BLIND to (indices into the 16-feature
    # vector): word_count(0), event_size(1), process_count(7).
    # 2026-07-17 HPO: leave-one-out CV showed these three actively hurt the IF
    # (verbose-but-benign logs isolate as easily as attacks), while the AE
    # benefits from them — so the mask is IF-only. 5-fold AUC 0.935 → 0.963.
    IF_DROPPED_FEATURES = (0, 1, 7)

    def extract_features(self, event):
        """Extract the shared 16 alert features, minus the IF-only mask (13 kept)."""
        full = self._extract_features_full(event)
        keep = [i for i in range(full.shape[1]) if i not in self.IF_DROPPED_FEATURES]
        return full[:, keep]

    def _extract_features_full(self, event):
    ...
    def detect_anomaly(self, event):
        ...
        try:
            ...
            # Post-inference override for events that are DEFINITELY routine:
            # user/hardcoded benign rule IDs, plus attack_labels' safe rule IDs and
            # safe descriptions (PAM session open/close, admin actions…). The model
            # may score these HIGH when training under-represented them, but they
            # are not attacks — cap them so they don't read as HIGH/CRITICAL.
            if rule_id in self._effective_benign_ids:
                normalized_score = min(normalized_score, 35)
            elif is_routine_safe(event, self._effective_benign_ids):
